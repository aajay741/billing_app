...
// Load customers
$custStmt = $pdo->query("SELECT id, name FROM customers ORDER BY name");
$customers = $custStmt->fetchAll(PDO::FETCH_ASSOC);

// Load products
$prodStmt = $pdo->query("SELECT id, item_name, sku, tax_percent, selling_price FROM products WHERE is_active=1 ORDER BY item_name");
$products = $prodStmt->fetchAll(PDO::FETCH_ASSOC);

// Load company settings for auto invoice
$cs = $pdo->query("SELECT invoice_prefix, next_invoice_no FROM company_settings WHERE id=1")->fetch(PDO::FETCH_ASSOC);
$invoice_prefix = $cs['invoice_prefix'] ?? '';
$next_invoice_no = (int)($cs['next_invoice_no'] ?? 1);

// helper
function oldSales($name, $default='') {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // invoice no always AUTO from settings
    $currentInvoiceNo = $invoice_prefix . $next_invoice_no;
    ...
    if ($customer_id <= 0) $errors[] = "Customer required.";
    ...
    // Prevent duplicate auto invoice if someone already used same no
    if (!$errors) {
        $chk = $pdo->prepare("SELECT id FROM sales_invoices WHERE invoice_no=:no LIMIT 1");
        $chk->execute(['no'=>$currentInvoiceNo]);
        if ($chk->fetchColumn()) {
            $errors[] = "Auto invoice number conflict. Reload page and try again.";
        }
    }
    ...
    if (!$errors) {
        ...
        $total_tax = 0;
        foreach ($lines as &$ln) {
            $lineTax = $ln['net_amount'] * $ln['tax_percent'] / 100; // GST = CGST+SGST
        ...
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO sales_invoices
                (customer_id, invoice_no, invoice_date, total_amount, total_tax, grand_total, payment_type)
                VALUES
                (:cid, :ino, :idate, :ta, :tt, :gt, :ptype)");
            $stmt->execute([
                'cid'   => $customer_id,
                'ino'   => $currentInvoiceNo,
            ...
            // Insert items + stock ledger (qty_out)
            $itemStmt = $pdo->prepare("INSERT INTO sales_items
                (sales_id, product_id, qty, rate, discount, tax_percent, amount)
                VALUES
                (:sid, :prid, :q, :r, :d, :t, :amt)");
            ...
            $ledgerStmtIns = $pdo->prepare("INSERT INTO stock_ledger
                ...
                VALUES (:prid, 'sale', :refid, 0, :qout, :cs)");
            ...
            // Update next_invoice_no in settings (AUTO increment)
            $upd = $pdo->prepare("UPDATE company_settings SET next_invoice_no = next_invoice_no + 1 WHERE id=1");
            $upd->execute();

            $pdo->commit();
            $success = "Sales invoice saved (No: {$currentInvoiceNo})";
            $_POST = [];
            // reload settings for next form
            $cs = $pdo->query("SELECT invoice_prefix, next_invoice_no FROM company_settings WHERE id=1")->fetch(PDO::FETCH_ASSOC);
            $invoice_prefix = $cs['invoice_prefix'] ?? '';
            $next_invoice_no = (int)($cs['next_invoice_no'] ?? 1);
        } catch (Exception $e) {
            $pdo->rollBack();
            $errors[] = "Error saving sales: " . $e->getMessage();
        }
    }
}
...
// ---------- LIST SALES INVOICES ----------
$q = trim($_GET['q'] ?? '');
$listSql = "FROM sales_invoices si
            JOIN customers c ON c.id = si.customer_id
            WHERE 1";
$params = [];

if ($q !== '') {
    $listSql .= " AND (si.invoice_no LIKE :q OR c.name LIKE :q)";
    $params['q'] = "%$q%";
}
...
$listStmt = $pdo->prepare("SELECT si.*, c.name AS customer_name
                           $listSql
                           ORDER BY si.id DESC
...
// current auto invoice no (after any updates above)
$currentAutoNo = $invoice_prefix . $next_invoice_no;
?>

<div class="p-3 border rounded mb-3 bg-white shadow-sm">
    <h6 class="mb-3">New Sales Invoice</h6>
    <form method="post" class="small" id="sales-form">
        <div class="row g-2 mb-2">
            ...
            <div class="col-md-3">
                ...
                <input type="text" class="form-control form-control-sm" value="<?=$currentAutoNo?>" readonly>
            </div>
            <div class="col-md-2">
                <label>Date*</label>
                <input type="date" name="invoice_date" class="form-control form-control-sm"
                       required value="<?=oldSales('invoice_date', date('Y-m-d'))?>">
            </div>
            <div class="col-md-2">
                <label>Payment</label>
                <select name="payment_type" class="form-select form-select-sm">
                    <option value="credit" <?=(oldSales('payment_type','credit')=='credit'?'selected':'')?>>Credit</option>
                    <option value="cash"   <?=(oldSales('payment_type')=='cash'?'selected':'')?>>Cash</option>
                </select>
            </div>
        </div>
        ...
        <div class="mt-2">
            <button class="btn btn-dark btn-sm">Save Sales</button>
        </div>
    </form>
</div>
...
<div class="d-flex justify-content-between align-items-center mb-2 small">
    <form method="get" class="d-flex">
        ...
        <input type="text" name="q" class="form-control form-control-sm"
               value="<?=htmlspecialchars($q)?>" placeholder="Search customer or invoice no">
...
<table class="table table-sm table-hover mb-0 small align-middle">
    ...
    <tbody>
    <?php if (!$rows): ?>
        <tr><td colspan="9" class="text-center py-3">No sales</td></tr>
        ...
        <tr>
            ...
            <td>
                <a class="btn btn-sm btn-outline-dark"
                   href="index.php?page=sales_view&id=<?=$r['id']?>">View</a>
            </td>
        </tr>
...
<?php for($i=1; $i<=$totalPages; $i++): ?>
    <li class="page-item <?=$i==$pageNum?'active':''?>">
        <a class="page-link"
           href="index.php?page=sales&p=<?=$i?>&q=<?=urlencode($q)?>"><?=$i?></a>
    </li>
<?php endfor; ?>
</ul>
</nav>
